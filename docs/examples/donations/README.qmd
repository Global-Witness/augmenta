...
- Because our dataset is UK-centric, we're setting the country to "GB" in Brave. This should help to filter some of the irrelevant results.
...
Make sure you have `augmenta` [installed](/README.md), open the terminal and navigate to the directory where you saved the data, configuration file and API keys.
...
This is particularly an issue with individuals with generic names. For example, it's likely that Patricia Bell in the dataset is not the University of Georgia professor surfaced by the search engine and classified by the LLM. This doesn't stop the LLM from offering a high degree of confidence in its classification.
